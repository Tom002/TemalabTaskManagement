...
import { Grid, Card, Button, Transition } from 'semantic-ui-react'
...
const TasksGrid :React.FC = () => {
    ...
    const {stateRegistry, tasksByOrder, deleteState, taskRegistry, editTask} = taskStore;
                ...
                <Droppable droppableId={String(state.stateId)}>
                  {
                    (provided,snapshot) => (
                      ...
                      >
                        ...
                        {tasksByOrder.filter(a => a.stateId === state.stateId).map(task =>(
